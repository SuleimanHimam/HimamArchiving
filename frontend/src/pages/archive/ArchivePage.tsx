...
export default function ArchivePage() {
  ...
  async function addLocation(e: React.FormEvent) {
    ...
    try {
      ...
      await loadLocations()
    } catch (err) { setError((err as AxiosError<{ error?: string }>).response?.data?.error ?? 'تعذّر إضافة الموقع') }
  }

  async function addItem(e: React.FormEvent) {
    e.preventDefault(); setError('')
    if (!item.physicalLocationId || !item.documentId) { setError('الموقع ورقم الوثيقة مطلوبان'); return }
    try {
      ...
      await loadItems()
    } catch (err) { setError((err as AxiosError<{ error?: string }>).response?.data?.error ?? 'تعذّر أرشفة البند') }
  }
      ...
      <div className="detail-grid">
        ...
        <motion.section className="doc-card" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <table className="reg-table">
            <thead><tr><th>الاسم</th><th>النوع</th><th>الرمز</th></tr></thead>
            <tbody>
              {locations.length === 0 && <tr><td colSpan={3} className="reg-empty">لا توجد مواقع</td></tr>}
              {locations.map((l) => (
                <tr key={l.id}><td>{l.name}</td><td>{LOCATION_TYPE_LABELS[['Building','Room','Cabinet','Shelf','Box'].indexOf(l.type)] ?? l.type}</td><td className="mono">{l.code ?? '—'}</td></tr>
              ))}
            </tbody>
          </table>
          ...
          {canCreate && (
            <form className="form-grid" onSubmit={addLocation} style={{ marginTop: '1rem' }}>
              ...
              <div className="form-actions"><button className="btn btn-primary">+ إضافة موقع</button></div>
            </form>
          )}
        </motion.section>
        ...
        <motion.section className="doc-card" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.08 }}>
          <h3 className="detail-h3">البنود المؤرشفة</h3>
          ...
          <table className="reg-table" style={{ marginTop: '.6rem' }}>
            <thead><tr><th>الوثيقة</th><th>الموقع</th><th>صندوق</th><th>ملف</th></tr></thead>
            <tbody>
              {items.length === 0 && <tr><td colSpan={4} className="reg-empty">لا توجد بنود</td></tr>}
              {items.map((it) => (
                <tr key={it.id}>
                  <td className="mono">{it.documentId ? `وثيقة #${it.documentId}` : it.incomingMailId ? `وارد #${it.incomingMailId}` : '—'}</td>
          ...
          {canArchive && (
            <form className="form-grid" onSubmit={addItem} style={{ marginTop: '1rem' }}>
              ...
              <div className="form-actions"><button className="btn btn-seal">أرشفة بند</button></div>
            </form>
          )}
        </motion.section>
      </div>
